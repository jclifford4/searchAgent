...
class GreedyAgent(Agent):
    "An agent that attempts to maximize its score as quickly as possible without planning"
    ...
    def getAction(self, state):
        ...
        legal = state.getLegalPacmanActions()
        # Do not allow the pacman to stop
        if Directions.STOP in legal: legal.remove(Directions.STOP)

        # Determine what next actions are possible and score each of them
        successors = [(state.generateSuccessor(0, action), action) for action in legal]
        scored = [(self.evaluationFunction(state), action) for state, action in successors]

        # Pick the best action, select randomly to break ties
