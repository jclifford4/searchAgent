...
class TimidAgent(Agent):
    ...
    """
    breakOnPurpose = False  # simulate error

    def __init__(self):
        super().__init__()
        # provides actions for when Pacman is not in danger
    ...
    def getAction(self, state):
        ...
        # Access the Pacman and determine where it is
        me = state.getPacmanState()
        mypos = me.getPosition()

        others = state.getGhostStates()  # access the other agents (ghosts)
        legal = state.getLegalPacmanActions()  # Determine what we are allowed to do
        ...
        if action is None:
            # Conditions were such that we did not select an action
            # Provide a behavior according to another agent model.
            action = self.secondaryAgent.getAction(state)
            if self.breakOnPurpose:  # for testing grading code
                action = Directions.LEFT[action]
                self.breakOnPurpose = False
